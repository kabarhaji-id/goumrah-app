...
const navigationItems = [{label: "Beranda", path: "/"}, {label: "Paket", path: "/paket"}, {
    label: "Blog",
    path: "/blog"
}, {label: "Goumrah Care", path: "/care"}, {label: "Tentang Kami", path: "/tentang"},];
...
export default function Navbar() {
    ...
    return (<>
            <header className="bg-white shadow-md sticky top-0 z-50">
                <div
                    className="container max-w-screen-xl mx-auto flex justify-between items-center h-[62px] px-6 md:px-10 lg:px-16">
                    <Link href="/" aria-label="Home" className="flex items-center">
                        <LogoDark className="w-auto h-auto"/>
                    </Link>

                    {screenType === "desktop" && (<nav className="flex gap-4 items-center" aria-label="Main navigation">
                            {navigationItems.map((item) => (<Link
                                    key={item.label}
                                    href={item.path}
                                    className={`px-3 py-2 text-sm font-medium ${pathname === item.path ? "font-bold text-teal-600" : "text-gray-800"} rounded-md hover:bg-teal-50 hover:text-teal-600 transition-all`}
                    ...
                    {screenType === "desktop" && (<div className="relative" ref={profileRef}>
                            {!user ? (<button
                                ...
                                >
                                    ...
                                    {isProfileOpen &&
                                        <ProfileMenu role={user.role} handleLogout={handleLogout} user={user}/>}
                                </>)}
                        </div>)}
            ...
            {/* ✅ Mobile & Tablet Profile Menu (without Avatar) */}
            {(screenType === "mobile" || screenType === "tablet") && isMobileMenuOpen && (
